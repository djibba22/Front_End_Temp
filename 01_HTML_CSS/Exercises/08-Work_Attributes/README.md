...
* It's done when the existing image's `src` attribute specifies the relative path to the `knightro.jpg` file found in the `assets` directory and the `alt` attribute describes the image.
...
![The portfolio page features headings in blue text, rounded corners on the image placeholder, and various background colors.](./assets/example.png)
